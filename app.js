...
const STORE_KEY="bearTravelPlanner_v22";
...
/* trips */
function tripList(){return `<section class="trip-list-head"><div><p class="eyebrow">MY TRIPS</p><h2>🧸 我的旅程</h2><p class="small">把每一趟期待，慢慢收進旅行手帳裡。</p></div><button class="btn primary add-trip-button" onclick="openAddTripModal()">＋新增旅程</button></section><div class="trip-list-grid">${state.trips.map((t,i)=>`<section class="card trip-card"><div class="trip-card-open" onclick="currentTripId=${t.id};currentTab=0;render()"><span class="trip-card-season">${seasonEmoji(t.startDate)}</span><div><p class="eyebrow">${esc(t.mainCurrency||"JPY")} · ${t.days.length} DAYS</p><h2>${esc(t.name)}</h2><p class="trip-date">${esc(t.date||"未設定日期")}</p><p class="small">${t.members.length?esc(t.members.join("、")):"尚未設定旅伴"}</p></div></div><div class="trip-card-actions"><button class="btn" onclick="editTrip(${i})">編輯</button><button class="btn danger" onclick="delTrip(${i})">刪除</button></div></section>`).join("")}</div>`}
function seasonEmoji(date){const m=Number(String(date||"").slice(5,7))||new Date().getMonth()+1;if([3,4,5].includes(m))return "🌸";if([6,7,8].includes(m))return "✨";if([9,10,11].includes(m))return "🍁";return "❄️"}
function openAddTripModal(){openModal(`<h2>＋新增旅程</h2><p class="small">先建立基本資料，之後都能再編輯。</p><input id="tn" placeholder="旅程名稱"><input id="td" placeholder="日期顯示，例如：2026.10.10－10.14"><input id="ts" type="date"><input id="tm" placeholder="旅伴，用逗號分隔"><input id="tnum" type="number" min="1" value="5" placeholder="天數"><select id="tc"><option value="JPY">日幣 JPY</option><option value="TWD">台幣 TWD</option><option value="KRW">韓幣 KRW</option><option value="THB">泰幣 THB</option><option value="USD">美金 USD</option><option value="EUR">歐元 EUR</option></select><button class="btn primary full-btn" onclick="addTrip()">建立旅程</button><button class="btn full-btn" onclick="closeModal()">取消</button>`)}
function addTrip(){const t=defTrip();t.name=tn.value||"未命名旅程";t.date=td.value||"";t.startDate=ts.value||"";t.mainCurrency=tc.value;t.members=tm.value?tm.value.split(",").map(x=>x.trim()).filter(Boolean):[];t.days=Array.from({length:Math.max(1,Number(tnum.value||5))},()=>[]);t.dayMeta=Array.from({length:t.days.length},()=>({weather:"",outfit:""}));state.trips.push(t);currentTripId=t.id;currentTab=0;closeModal();render()}
...
function flightBlock(title,x){return `<h3 class="flight-direction">${title}</h3><div class="flight-card compact"><div class="flight-airline"><span>✈︎</span><div><small>AIRLINE</small><b>${esc(x.airline||"航空公司")} ${esc(x.no||"")}</b></div></div><div class="flight-route"><div><div class="airport">${esc(x.from||"TPE")}</div><small>出發</small></div><div class="flight-path"><span></span><b>→</b><span></span></div><div><div class="airport">${esc(x.to||"FUK")}</div><small>抵達</small></div></div><div class="flight-info flight-info-v22"><div><small>航班時間</small><b>${esc(x.time||"未填")}</b></div><div><small>飛行時間</small><b>${esc(x.duration||"未填")}</b></div><div class="flight-note"><small>備註</small><b>${esc(x.note||"—")}</b></div></div></div>`}
function editFlight(which){const x=trip().flight[which];openModal(`<h2>編輯${which==="go"?"去程":"回程"}航班</h2><input id="fAirline" placeholder="航空公司" value="${esc(x.airline)}"><input id="fNo" placeholder="航班號" value="${esc(x.no)}"><div class="grid"><input id="fFrom" placeholder="出發機場代碼" value="${esc(x.from)}"><input id="fTo" placeholder="抵達機場代碼" value="${esc(x.to)}"></div><input id="fTime" placeholder="航班時間" value="${esc(x.time)}"><input id="fDuration" placeholder="飛行時間" value="${esc(x.duration)}"><textarea id="fNote" placeholder="備註">${esc(x.note)}</textarea><button class="btn primary full-btn" onclick="saveFlight('${which}')">儲存</button><button class="btn full-btn" onclick="closeModal()">取消</button>`)}
...
function card(x,i,d,list){const move=(i<list.length-1&&x.moveTime)?`<div class="move-line">${esc(x.trans||"🚗 開車")} 約 ${esc(x.moveTime)}</div>`:"";const cover=x.photo?`<img class="it-thumb" src="${x.photo}" ${imgAttrs(x.photo)}>`:`<button class="it-thumb cover-pick" onclick="openCoverPicker(${d},${i})">🖼<small>封面</small></button>`;return `<div class="timeline-entry"><div class="timeline-time">${esc(fmtTime(x.arrive)||"—")}</div><section class="it-card ${itemClass(x)}" data-idx="${i}" draggable="true" ondragstart="window.dragIndex=${i};this.classList.add('dragging')" ondragend="this.classList.remove('dragging')" ondragover="event.preventDefault();this.classList.add('drop-target')" ondragleave="this.classList.remove('drop-target')" ondrop="dropItem(${d},${i})"><div class="it-body"><div class="it-top"><div class="it-main"><div class="it-title-row"><div class="it-title"><span class="inline-icon">${typeIcon(x.type)}</span>${esc(x.name||"未命名")}</div></div><div class="chips">${x.stay?`<span class="chip">⏱ ${esc(x.stay)}</span>`:""}${x.rating?`<span class="chip">⭐ ${esc(x.rating)}</span>`:""}</div></div>${cover}</div>${x.memo?`<p class="note">${esc(x.memo)}</p>`:""}<div class="card-actions-row">${x.map?`<a class="btn green" href="${esc(x.map)}" target="_blank">導航</a>`:`<button class="btn green" onclick="alert('尚未填寫導航連結')">導航</button>`}<button class="btn" onclick="editItem(${d},${i})">編輯</button><button class="btn" onclick="openItemMoveModal(${d},${i})">複製/移動</button><button class="btn danger" onclick="delItem(${d},${i})">刪除</button><span class="drag-grip" ontouchstart="startTouchDrag(event,${d},${i})" ontouchmove="moveTouchDrag(event)" ontouchend="endTouchDrag(event)">≡</span></div></div></section></div>${move}`}
...
function notesPage(){const t=trip();return `<section class="card note-create-card"><h2>📝 備忘錄</h2><input id="noteTitle" placeholder="標題，例如：唐吉軻德"><textarea id="noteText" placeholder="備忘內容"></textarea><input id="noteMap" placeholder="導航或網址：https://..."><label class="upload-box">📷 加入圖片<input id="notePhoto" type="file" accept="image/*" onchange="previewFile(this,'notePreview')" hidden></label><div id="notePreview"></div><button class="btn primary full-btn" onclick="addNote()">新增備忘</button></section><div class="notes-grid">${(t.notes||[]).map((n,i)=>`<section class="card note-card">${n.photo?`<img class="note-photo" src="${n.photo}" ${imgAttrs(n.photo)}>`:""}<h3>${esc(n.title||'備忘')}</h3><p class="note">${esc(n.text||'')}</p><div class="card-actions-row">${n.map?`<a class="btn green" href="${esc(n.map)}" target="_blank">開啟連結</a>`:""}<button class="btn" onclick="editNote(${i})">編輯</button><button class="btn danger" onclick="delNote(${i})">刪除</button></div></section>`).join('')}</div>`}
function addNote(){readImage(notePhoto,photo=>{trip().notes.push({title:noteTitle.value,text:noteText.value,map:noteMap.value,photo});render()})}
function editNote(i){const n=trip().notes[i];openModal(`<h2>編輯備忘</h2><input id="neTitle" value="${esc(n.title)}"><textarea id="neText">${esc(n.text)}</textarea><input id="neMap" placeholder="導航或網址" value="${esc(n.map||'')}">${n.photo?`<img class="receipt-preview" src="${n.photo}" ${imgAttrs(n.photo)}>`:""}<label class="upload-box">📷 更換圖片<input id="nePhoto" type="file" accept="image/*" hidden></label><button class="btn primary full-btn" onclick="saveNoteEdit(${i})">儲存</button><button class="btn full-btn" onclick="closeModal()">取消</button>`)}
...
function initSeasonalDecor(){
  let layer=document.getElementById("seasonalDecor");
  if(!layer){layer=document.createElement("div");layer.id="seasonalDecor";document.body.appendChild(layer)}
  const m=new Date().getMonth()+1;
  const symbol=[3,4,5].includes(m)?"🌸":[6,7,8].includes(m)?"✦":[9,10,11].includes(m)?"🍂":"❄";
  layer.innerHTML=Array.from({length:12},(_,i)=>`<span style="--i:${i};--x:${(i*37)%100};--delay:${(i%6)*-1.7}s">${symbol}</span>`).join("");
}
