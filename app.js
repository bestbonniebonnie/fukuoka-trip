...
function openModal(h){modal.classList.remove("image-preview-mode");modalBody.innerHTML=h;modal.classList.remove("hidden")}
function closeModal(){modal.classList.add("hidden");modal.classList.remove("image-preview-mode");modalBody.innerHTML=""}
function previewImage(src){if(!src)return;openModal(`<div class="preview-head"><h2>圖片預覽</h2><button class="modal-x" onclick="closeModal()">×</button></div><img class="full-preview" src="${src}"><p class="small preview-tip">點右上角 × 或外側背景即可關閉。</p>`);modal.classList.add("image-preview-mode")}
...
/* overview */
function overview(){const t=trip();return `<section class="trip-hero"><h2>${esc(t.name)}</h2><p>${esc(t.date)}</p><p>${t.days.length}天｜${t.members.length}位旅伴｜${t.days.flat().length}個行程｜${t.mainCurrency}</p></section><section class="card"><h3>旅伴設定</h3><input id="members" value="${esc(t.members.join(","))}"><button class="btn primary" onclick="trip().members=members.value.split(',').map(x=>x.trim()).filter(Boolean);render()">更新旅伴</button><h3>主要幣別</h3><select id="mainCur">${availableCurrencies().map(c=>`<option value="${c}" ${c===(t.mainCurrency||"JPY")?"selected":""}>${c}</option>`).join("")}</select><button class="btn primary" onclick="trip().mainCurrency=mainCur.value;render()">更新幣別</button><h3>天數設定</h3><button class="btn dark" onclick="trip().days.push([]);render()">＋增加一天</button><button class="btn danger" onclick="removeDay()">－減少一天</button></section>`}
...
function flightBlock(title,x){return `<h3>${title}</h3><div class="flight-card compact"><div class="flight-brand"><span>${esc(x.airline||"航空公司")}</span><b>${esc(x.no||"航班號")}</b></div><div class="flight-route"><div class="flight-stop"><time>${esc(fmtTime(x.departTime)||"--:--")}</time><div class="airport">${esc(x.from||"TPE")}</div></div><div class="flight-path"><span>✈</span></div><div class="flight-stop"><time>${esc(fmtTime(x.arriveTime)||"--:--")}</time><div class="airport">${esc(x.to||"FUK")}</div></div></div><div class="flight-info two"><div>飛行時間<b>${esc(x.duration||"未填")}</b></div><div class="flight-note-cell">備註<b>${esc(x.note||"—")}</b></div></div></div>`}
function editFlight(which){const x=trip().flight[which];openModal(`<h2>編輯${which==="go"?"去程":"回程"}航班</h2><input id="fAirline" placeholder="航空公司" value="${esc(x.airline)}"><input id="fNo" placeholder="航班號" value="${esc(x.no)}"><div class="grid"><input id="fFrom" placeholder="出發機場代碼" value="${esc(x.from)}"><input id="fTo" placeholder="抵達機場代碼" value="${esc(x.to)}"></div><div class="grid"><input id="fDepartTime" inputmode="numeric" placeholder="出發時間，例如 08:30" value="${esc(fmtTime(x.departTime))}"><input id="fArriveTime" inputmode="numeric" placeholder="抵達時間，例如 11:55" value="${esc(fmtTime(x.arriveTime))}"></div><input id="fDuration" placeholder="飛行時間" value="${esc(x.duration)}"><textarea id="fNote" placeholder="備註">${esc(x.note)}</textarea><button class="btn primary" onclick="saveFlight('${which}')">儲存</button><button class="btn" onclick="closeModal()">取消</button>`)}
function saveFlight(which){trip().flight[which]={airline:fAirline.value,no:fNo.value,from:fFrom.value,to:fTo.value,departTime:fmtTime(fDepartTime.value),arriveTime:fmtTime(fArriveTime.value),time:"",duration:fDuration.value,terminal:"",note:fNote.value};closeModal();render()}
...
function removeCurrentDay(i){if(trip().days.length<=1)return alert('至少保留一天');if(confirm(`確定刪除 Day ${i+1}？`)){trip().days.splice(i,1);trip().dayMeta.splice(i,1);currentTab=Math.min(currentTab,trip().days.length+1);render()}}
function openCategoryHint(){openModal(`<h2>分類設定</h2><p class="note">行程卡會依分類自動套用顏色：住宿為綠色、景點為奶茶橘、美食為珊瑚粉、購物為莓粉、咖啡廳為咖啡色。</p><button class="btn primary" onclick="closeModal()">知道了</button>`)}
function showTodayRoute(i){const list=trip().days[i]||[];const d=dayDate(i);const rows=list.length?list.map(x=>`<div class="route-row"><time>${esc(fmtTime(x.arrive)||'--:--')}</time><span>${esc(x.name||'未命名行程')}</span></div>`).join(''):`<p class="empty-route">這一天尚未安排行程</p>`;openModal(`<div class="route-modal-head"><div><span class="eyebrow">TODAY ROUTE</span><h2>今日路線</h2><p>Day ${i+1} · ${d.full}</p></div><button class="modal-x" onclick="closeModal()">×</button></div><div class="route-list">${rows}</div>`)}
...
function card(x,i,d,list){const move=(i<list.length-1&&x.moveTime)?`<div class="travel-step"><span>${esc(x.trans||"🚗 開車")}</span><b>${esc(x.moveTime)}</b></div>`:"";const cover=x.photo?`<img class="it-thumb" src="${x.photo}" ${imgAttrs(x.photo)}>`:"";return `<div class="timeline-entry" data-idx="${i}"><div class="time-rail"><time>${esc(fmtTime(x.arrive)||"--:--")}</time><i></i></div><section class="it-card ${itemClass(x)}" draggable="true" ondragstart="window.dragIndex=${i};this.classList.add('dragging')" ondragend="this.classList.remove('dragging')" ondragover="event.preventDefault();this.classList.add('drop-target')" ondragleave="this.classList.remove('drop-target')" ondrop="dropItem(${d},${i})"><div class="card-order"><button onclick="moveTo(${d},${i},${Math.max(0,i-1)})">⌃</button><span class="drag-grip" ontouchstart="startTouchDrag(event,${d},${i})" ontouchmove="moveTouchDrag(event)" ontouchend="endTouchDrag(event)">⠿</span><button onclick="moveTo(${d},${i},${Math.min(list.length-1,i+1)})">⌄</button></div><div class="it-body"><div class="it-top"><div class="it-main"><div class="chips"><span class="category-pill">${typeIcon(x.type)} ${(x.type||'景點').replace(/^[^ ]+\s*/, '')}</span>${x.stay?`<span class="stay-pill">停留 ${esc(x.stay)}</span>`:""}</div><div class="it-title">${esc(x.name||"未命名")}</div>${x.memo?`<p class="it-memo">${esc(x.memo)}</p>`:""}</div>${cover}<div class="card-top-actions"><button class="more-btn" onclick="editItem(${d},${i})">⋮</button></div></div><div class="card-actions-row">${x.map?`<a class="nav-pill" href="${esc(x.map)}" target="_blank">⌖ Google 導航</a>`:`<button class="nav-pill" onclick="alert('尚未填寫導航連結')">⌖ Google 導航</button>`}<button class="mini-action" onclick="openItemMoveModal(${d},${i})">複製／移動</button></div></div></section>${move}</div>`}
...
function editItem(d,i){openModal(form(trip().days[d][i],`saveItem(${d},${i})`,`delItemFromEditor(${d},${i})`))}
...
function delItemFromEditor(d,i){if(confirm("確定要刪除此行程嗎？")){trip().days[d].splice(i,1);closeModal();render()}}
...
/* money */
function rateBox(){const t=trip(),c=t.mainCurrency||"JPY";return `<section class="card rate-card"><h3>匯率設定</h3><p class="small">全站金額以 ${c} 為主，台幣只作輔助換算。</p>${availableCurrencies().map(x=>`<label>${x} → TWD<input type="number" step="0.0001" value="${t.rates[x]??1}" onchange="trip().rates['${x}']=Number(this.value||1);render()"></label>`).join('')}<button class="btn" onclick="openAddCurrency()">＋ 新增幣別</button></section>`}
function openAddCurrency(){openModal(`<h2>新增幣別</h2><input id="newCurrencyCode" maxlength="6" placeholder="幣別代碼，例如 SGD / CNY"><input id="newCurrencyRate" type="number" step="0.0001" placeholder="1 單位等於多少 TWD"><button class="btn primary" onclick="addCustomCurrency()">新增</button><button class="btn" onclick="closeModal()">取消</button>`)}
function addCustomCurrency(){const code=String(newCurrencyCode.value||'').trim().toUpperCase();const rate=Number(newCurrencyRate.value||0);if(!/^[A-Z]{2,6}$/.test(code))return alert('請輸入 2～6 碼英文字母幣別代碼');if(!(rate>0))return alert('請輸入正確匯率');trip().rates[code]=rate;closeModal();render()}
...
function expenses(){
  ...
  return `${rateBox()}<section class="card page-toolbar"><div><h2>💰 記帳</h2><div class="total-money"><b>${c} 總額</b><div class="money">${fmtMoney(totalMain,c)}</div><p class="small">約 NT$ ${Math.round(totalTwd).toLocaleString()}</p></div></div><div class="toolbar-actions"><button class="btn" onclick="receiptScan()">📷 掃描收據</button><button class="btn primary" onclick="openExpenseForm()">＋ 新增</button></div></section><div class="expense-grid">${t.expenses.map((x,i)=>`<section class="card expense-card">${x.receipt?`<img class="expense-card-photo" src="${x.receipt}" ${imgAttrs(x.receipt)}>`:""}<h3>${esc(x.name)}</h3>${moneyTextTrip(x.amount,x.cur||c)}${x.memo?`<p class="note">${esc(x.memo)}</p>`:""}<div class="compact-actions"><button class="btn" onclick="editExpense(${i})">編輯</button><button class="btn danger" onclick="delExpense(${i})">刪除</button></div></section>`).join("")}</div>`
}
function openExpenseForm(){const c=trip().mainCurrency||"JPY";openModal(`<h2>新增記帳</h2><input id="exName" placeholder="項目名稱"><input id="exAmount" type="number" placeholder="金額（${c}）"><textarea id="exMemo" placeholder="備註"></textarea><input id="exReceipt" type="file" accept="image/*" onchange="previewFile(this,'expensePreview')"><div id="expensePreview"></div><button class="btn primary" onclick="addExpense()">儲存</button><button class="btn" onclick="closeModal()">取消</button>`)}
...
function toolsPage(){const t=trip(),c=t.mainCurrency||'JPY';return `<section class="card"><h2>⚙️ 工具</h2><p class="small">正式使用前建議先按「一鍵完整備份」。資料存在手機瀏覽器，下載備份後換手機也可以匯入還原。</p><div class="tool-grid"><button class="btn primary" onclick="exportTrip()">匯出這趟旅程</button><button class="btn primary" onclick="exportAll()">一鍵完整備份</button><button class="btn" onclick="openImport()">匯入／還原</button><button class="btn" onclick="duplicateTrip()">複製旅程</button><button class="btn" onclick="printTrip()">匯出 PDF / 列印</button></div><p class="small">目前版本：Bear Travel Planner v24.2</p></section><section class="card"><h2>💱 匯率換算</h2><div class="grid"><input id="cvAmount" type="number" placeholder="金額，例如 1000"><select id="cvCur">${availableCurrencies().map(x=>`<option value="${x}" ${x===c?'selected':''}>${x}</option>`).join('')}</select></div><button class="btn primary" onclick="convertMoney()">換算台幣</button><div id="cvResult" class="note">輸入金額後按換算。</div><div class="rate-title-row"><h3>匯率設定</h3><button class="btn" onclick="openAddCurrency()">＋ 新增幣別</button></div><p class="small">更改匯率後，記帳與分攤的「約 NT$」會重新換算，原始旅程幣別金額不會被改掉。</p>${availableCurrencies().map(x=>`<label>${x} → TWD<input type="number" step="0.0001" value="${t.rates[x]??1}" onchange="trip().rates['${x}']=Number(this.value||1);render()"></label>`).join('')}</section>`}
...
function notesPage(){const t=trip();return `<section class="card page-toolbar"><h2>📝 備忘錄</h2><button class="btn primary" onclick="openNoteForm()">＋ 新增</button></section><div class="note-grid">${(t.notes||[]).map((n,i)=>`<section class="card note-card">${n.photo?`<img class="note-photo" src="${n.photo}" ${imgAttrs(n.photo)}>`:''}<h3>${esc(n.title||'備忘')}</h3><p class="note">${esc(n.text||'')}</p>${n.map?`<a class="btn green" target="_blank" href="${esc(n.map)}">開啟導航／連結</a>`:''}<div class="compact-actions"><button class="btn" onclick="editNote(${i})">編輯</button><button class="btn danger" onclick="delNote(${i})">刪除</button></div></section>`).join('')}</div>`}
function openNoteForm(){openModal(`<h2>新增備忘</h2><input id="noteTitle" placeholder="標題，例如：唐吉軻德"><textarea id="noteText" placeholder="備忘內容"></textarea><input id="noteMap" placeholder="導航／網址連結（選填）"><input id="notePhoto" type="file" accept="image/*"><button class="btn primary" onclick="addNote()">儲存</button><button class="btn" onclick="closeModal()">取消</button>`)}
...
function luggagePage(){const t=trip(),groups={};(t.luggage||[]).forEach((x,i)=>{const o=x.owner||'未分類';(groups[o]||(groups[o]=[])).push({...x,i})});return `<section class="card"><h2>🧳 行李清單</h2><details class="add-panel"><summary>＋ 新增</summary><div class="add-panel-body"><div class="grid"><input id="lugOwner" placeholder="對象，例如：媽媽 / 樂寶 / 共用"><input id="lugItem" placeholder="物品，例如：尿布 / 充電器"></div><textarea id="lugMemo" placeholder="備註"></textarea><button class="btn primary" onclick="addLuggage()">新增行李</button></div></details></section>${Object.keys(groups).sort().map(o=>`<section class="card"><h3>${esc(o)}</h3>${groups[o].map(x=>`<label class="check-row ${x.done?'done':''}"><input type="checkbox" ${x.done?'checked':''} onchange="toggleLuggage(${x.i})"><span><b>${esc(x.item)}</b>${x.memo?`<small>${esc(x.memo)}</small>`:''}</span><button class="delete-mini" onclick="event.preventDefault();delLuggage(${x.i})">刪除</button></label>`).join('')}</section>`).join('')}`}
function addLuggage(){trip().luggage.push({owner:lugOwner.value||'共用',item:lugItem.value,memo:lugMemo.value,done:false});render()}
