...
/* trips */
function tripList(){return `<section class="trip-list-head"><div><span class="eyebrow">MY TRIPS</span><h2>🧸 我的旅程</h2><p class="small">把每一趟旅行，慢慢收進這裡。</p></div><button class="btn primary create-trip-btn" onclick="openTripCreate()">＋ 新增旅程</button></section>${state.trips.map((t,i)=>`<section class="card trip-card"><div class="trip-card-main" onclick="currentTripId=${t.id};currentTab=0;render()"><span class="trip-card-tag">${t.days.length} DAYS</span><h2>${esc(t.name)}</h2><p>${esc(t.date||'尚未設定日期')}</p><p class="small">${t.mainCurrency||"JPY"}｜${t.members.join("、")||'尚未設定旅伴'}</p></div><div class="trip-card-actions"><button class="btn" onclick="editTrip(${i})">編輯</button><button class="btn danger" onclick="delTrip(${i})">刪除</button></div></section>`).join("")}`}
...
function day(i){const t=trip(),list=t.days[i],meta=t.dayMeta[i]||{weather:"",outfit:""},d=dayDate(i);return `<section class="day-stage"><div class="day-heading"><span class="day-label">🍁 DAY ${i+1}</span><h2>${d.full} <small>(${d.w})</small></h2></div><div class="date-strip">${t.days.map((_,idx)=>{const dd=dayDate(idx);return `<div class="date-chip ${idx===i?'active':''}" onclick="setTab(${idx+2})"><div class="date-main">${dd.w}</div><div class="week-main">${dd.d}</div></div>`}).join("")}<button class="date-chip add-day-chip" onclick="trip().days.push([]);trip().dayMeta.push({weather:'',outfit:''});render()"><span>＋</span><small>加一天</small></button></div><div class="day-meta-grid"><div class="mini-info-card"><div class="mini-info-head"><b>${weatherIcon(meta.weather)} 天氣</b><button class="tiny-edit" onclick="editDayMeta(${i},'weather')">編輯</button></div><p>${esc(meta.weather||"點編輯輸入天氣")}</p></div><div class="mini-info-card"><div class="mini-info-head"><b>👕 穿著備註</b><button class="tiny-edit" onclick="editDayMeta(${i},'outfit')">編輯</button></div><p>${esc(meta.outfit||"點編輯輸入穿著備註")}</p></div></div><div class="day-command-row"><button onclick="openDayMoveModal(${i})">▦ 整趟總覽</button><button onclick="openCategoryHint()">⚙ 分類設定</button><button onclick="showTodayRoute(${i})">⌘ 今日路線</button></div></section><div class="filters">${["全部","景點","住宿","美食","購物"].map(f=>`<button class="${filterType===f?'on':''}" onclick="filterType='${f}';render()">${f}</button>`).join("")}<button class="add-filter" onclick="openItForm(${i})">＋ 新增行程</button><button class="day-tool-btn danger-soft" onclick="removeCurrentDay(${i})">刪除此天</button></div>${filteredList(list).length?`<div class="timeline">${filteredList(list).map(x=>card(x,list.indexOf(x),i,list)).join("")}</div>`:`<section class="card empty">這一天還沒有行程，按「新增行程」開始安排 🍂</section>`}`}
...
function notesPage(){const t=trip();return `<section class="card"><h2>📝 備忘錄</h2><input id="noteTitle" placeholder="標題，例如：唐吉軻德"><textarea id="noteText" placeholder="備忘內容"></textarea><input id="noteMap" placeholder="導航／網址連結（選填）"><input id="notePhoto" type="file" accept="image/*"><button class="btn primary" onclick="addNote()">新增備忘</button></section>${(t.notes||[]).map((n,i)=>`<section class="card note-card">${n.photo?`<img class="note-photo" src="${n.photo}" ${imgAttrs(n.photo)}>`:''}<h3>${esc(n.title||'備忘')}</h3><p class="note">${esc(n.text||'')}</p>${n.map?`<a class="btn green" target="_blank" href="${esc(n.map)}">開啟導航／連結</a>`:''}<button class="btn" onclick="editNote(${i})">編輯</button><button class="btn danger" onclick="delNote(${i})">刪除</button></section>`).join('')}`}
